...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_lolos"]) ) {
    $id = $_POST["id"];
    ...
    try {
        ...
        if ($status_lolos == 'Siap Training') {
            ...
            $sql_update = "UPDATE crew3 SET status_lolos = ?, lolos_date = ? WHERE id = ?";
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Active Crew FF Batch 3 Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                <h2 style="font-size: 20px; color: #5cb85c; margin-bottom: 10px;">Dear Academy Team,</h2>
                                                <p>We would like to inform you that a new Active Crew FF Batch 3 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear Academy Team,'
                                                . 'We would like to inform you that a new Active Crew FF Batch 3 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            $conn->commit();
            echo "Status berhasil diperbarui.";
        }elseif ($status_lolos == 'Cancel') {
            $lolos_date = date("Y-m-d H:i:s");
            $status_oje = "Cancel";

            // Query untuk memperbarui status status_lolos di tabel draft
            $sql_update = "UPDATE crew3 SET status_lolos = ?, lolos_date = ?, status_oje = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_lolos, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE crew3 SET status_lolos = ?, lolos_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("ssi", $status_lolos, $lolos_date, $id);
                ...
                $sql = "UPDATE crew3 SET status_lolos = ?, lolos_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_lolos, $lolos_date, $id);
                $stmt->execute();
        ...
        if (isset($kode_lahan) && !empty($kode_lahan)) {
            header("Location: ../datatables-data-ff3.php?id=" . urlencode($kode_lahan));
            exit;
        } else {
