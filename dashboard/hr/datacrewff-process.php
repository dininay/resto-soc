...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_lolos"]) ) {
    ...
    try {
        ...
        if ($status_lolos == 'Siap Training') {
            ...
            $sql_update = "UPDATE crew SET status_lolos = ?, lolos_date = ? WHERE id = ?";
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Active Crew FF Batch 1 Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Active Crew FF Batch 1 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear Academy Team,'
                                                . 'We would like to inform you that a new Active Crew FF Batch 1 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            // Query untuk memperbarui status status_lolos di tabel draft
            $sql_update = "UPDATE crew SET status_lolos = ?, lolos_date = ?, status_oje = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            ...
            echo "Status berhasil diperbarui.";
                ...
                $sql_update_re = "UPDATE crew SET status_lolos = ?, lolos_date = ? WHERE id = ?";
                ...
                $sql = "UPDATE crew SET status_lolos = ?, lolos_date = ? WHERE id = ?";
        ...
        if (isset($kode_lahan) && !empty($kode_lahan)) {
            header("Location: ../datatables-data-ff1.php?id=" . urlencode($kode_lahan));
            exit;
        } else {
