...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_tm"]) && isset($_POST["catatan_tm"]) ) {
    $id = $_POST["id"];
    $status_tm = $_POST["status_tm"];
    $catatan_tm = $_POST["catatan_tm"];
    $tm_date = null;
    ...
    try {
        ...
        if ($status_tm == 'Done') {
            ...
            $sql_update = "UPDATE socdate_hr SET status_tm = ?, tm_date = ?, catatan_tm = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_tm, $tm_date, $catatan_tm, $id);
            $stmt_update->execute();
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_tm, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_hr SET status_tm = ?, tm_date = ?, catatan_tm = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_tm, $tm_date, $catatan_tm, $id);
                ...
                $sql = "UPDATE socdate_hr SET status_tm = ?, tm_date = ?, catatan_tm = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_tm, $tm_date, $catatan_tm, $id);
                $stmt->execute();
