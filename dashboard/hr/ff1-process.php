...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_ff1"]) && isset($_POST["catatan_ff1"]) ) {
    $id = $_POST["id"];
    $status_ff1 = $_POST["status_ff1"];
    $catatan_ff1 = $_POST["catatan_ff1"];
    $ff1_date = null;
    ...
    try {
        ...
        if ($status_ff1 == 'Done') {
            ...
            $sql_update = "UPDATE socdate_hr SET status_ff1 = ?, ff1_date = ?, catatan_ff1 = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_ff1, $ff1_date, $catatan_ff1, $id);
            $stmt_update->execute();
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Prepare RTO by HR (FF Crew Batch 1) Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Prepare RTO by HR (FF Crew Batch 1) Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear PMO Team,'
                                                . 'We would like to inform you that a new Information Prepare RTO by HR (FF Crew Batch 1) Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_ff1, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_hr SET status_ff1 = ?, ff1_date = ?, catatan_ff1 = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_ff1, $ff1_date, $catatan_ff1, $id);
                ...
                $sql = "UPDATE socdate_hr SET status_ff1 = ?, ff1_date = ?, catatan_ff1 = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_ff1, $ff1_date, $catatan_ff1, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-hr-fulfillment.php");
