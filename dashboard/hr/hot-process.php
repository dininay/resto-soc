...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_hot"]) && isset($_POST["catatan_hot"]) ) {
    $id = $_POST["id"];
    ...
    try {
        ...
        if ($status_hot == 'Done') {
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Prepare RTO by HR (Hand Over Training) By TAF Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Prepare RTO by HR (Hand Over Training) By TAF Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear Procurement Team,'
                                                . 'We would like to inform you that a new Information Prepare RTO by HR (Hand Over Training) By TAF Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_hot, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_hr SET status_hot = ?, hot_date = ?, catatan_hot = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_hot, $hot_date, $catatan_hot, $id);
                ...
                $sql = "UPDATE socdate_hr SET status_hot = ?, hot_date = ?, catatan_hot = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_hot, $hot_date, $catatan_hot, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-hr-hot.php");
