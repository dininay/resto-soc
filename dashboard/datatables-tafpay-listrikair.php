...
WHERE status_tafpay IN ('In Process', 'Paid', 'Pending')";
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_paypdam'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_paypdam_files as $listrik) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $listrik . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    $slaLegalDate = new DateTime($row['sla_tafpay']);
                                                    ...
                                                    if ($today->format('N') == 6 || $today->format('N') == 7) { // N = 6 is Saturday, N = 7 is Sunday
                                                        ...
                                                        // Menghitung jumlah hari Kamis, Sabtu, dan Minggu antara sla_tafpay dan hari ini
                                                        $interval = new DateInterval('P1D');
                                                        $period = new DatePeriod($slaLegalDate, $interval, $today);
                                                        $nonWorkingDays = 0;

                                                        foreach ($period as $date) {
                                                            if ($date->format('N') == 4 || $date->format('N') == 6 || $date->format('N') == 7) { // Thursday, Saturday, Sunday
                                                                $nonWorkingDays++;
                                                            }
                                                        }

                                                        // Menghitung selisih hari antara sla_tafpay dan hari ini
                                                        $diff = $slaLegalDate->diff($today);

                                                        // Mengurangi jumlah hari tidak bekerja dari total selisih hari
                                                        $effectiveDays = $diff->days - $nonWorkingDays;

                                                        // Menghitung scoring
                                                        $scoring = calculateScoring($row['tafpay_date'], $row['sla_tafpay'], $sla_value); // Make sure $sla_value is set correctly
                                                        $remarks = getRemarks($scoring);

                                                        if ($row['status_tafpay'] == "Paid") {
                                                            ...
                                                            // Menghitung apakah tanggal SLA telah terlewati atau masih dalam jangka waktu
                                                            if ($today > $slaLegalDate) {
                                                                // Jika terlambat
                                                                echo '<button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#lateApprovalModal">Terlewat ' . $effectiveDays . ' hari</button>';
                                                            } else {
                                                                // Jika masih dalam jangka waktu, tampilkan peringatan "H - X"
                                                                echo '<button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#deadlineModal">H - ' . $effectiveDays . '</button>';
                                                            }
                                                        }
                                                    }
                                                    ?>
                                                ...
                                                <td>
                                                <!-- Tombol Edit -->
                                                        <?php if ($row['status_tafpay'] != "Paid"): ?>
                                                        <a href="fat/tafpay-edit-form.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>
                                                        <?php endif; ?>
                                                        <?php if ($row['status_tafpay'] != "Paid"): ?>
                                                            <button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="<?= $row['id'] ?>" data-status="<?= $row['status_tafpay'] ?>">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="fat/tafpay-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status Payment TAF</label>
                                                                        <select class="form-control" id="statusSelect" name="status_tafpay" Placeholder="Pilih">
