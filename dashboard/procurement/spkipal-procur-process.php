...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])  && isset($_POST["status_spkwoipal"])&& isset($_POST["catatan_spkwoipal"])) {
    ...
    try {
        ...
        if ($stmt_update->execute() === TRUE) {
            ...
            if ($status_spkwoipal == 'In Review By TAF') {
                $spkwoipal_date = date("Y-m-d");
                ...
                $sql_update_pending = "UPDATE socdate_sdg SET status_spkwoipal = ?, catatan_spkwoipal = ?, spkwoipal_date = ?, status_spkwoipaltaf = ?, sla_spkwoipaltaf = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("sssssi", $status_spkwoipal, $catatan_spkwoipal, $spkwoipal_date, $status_spkwoipaltaf, $sla_spkwoipaltaf, $id);
