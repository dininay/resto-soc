...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])  && isset($_POST["status_jobadd"])&& isset($_POST["catatan_jobadd"])) {
    $id = $_POST["id"];
    $status_jobadd = $_POST["status_jobadd"];
    $catatan_jobadd = $_POST["catatan_jobadd"];
    ...
    try {
        // Query untuk memperbarui status_jobadd berdasarkan id
        $sql_update = "UPDATE jobadd SET status_jobadd = ?, catatan_jobadd = ?, jobadd_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $jobadd_date = date("Y-m-d");
        $stmt_update->bind_param("sssi", $status_jobadd, $catatan_jobadd, $jobadd_date, $id);

        // Eksekusi query update
        if ($stmt_update->execute() === TRUE) {
            // Jika status_jobadd diubah menjadi 'Approve'
            if ($status_jobadd == 'Approve') {
                ...
                if ($result_sla_spk->num_rows > 0) {
                    ...
                    $sla_spkfat = date("Y-m-d", strtotime($jobadd_date . ' + ' . $hari_sla_spk . ' days'));
                } else {
                    echo "Error: Data SLA tidak ditemukan untuk divisi SPK.";
                    exit;
                }
                $status_spkfat = "In Process";
                $status_wojobadd = "Approve";
                
                // Query untuk memperbarui submit_legal dan catatan_owner di tabel jobadd
                $sql_update_pending = "UPDATE jobadd SET status_jobadd = ?, catatan_jobadd = ?, jobadd_date = ?, status_wojobadd = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("ssssi", $status_jobadd, $catatan_jobadd, $jobadd_date, $status_wojobadd, $id);
            ...
            foreach ($departments as $department) {
                ...
                if (!empty($toEmails)) {
                    try {
                        ...
                        $mail->Subject = 'Notification: New SPK Job Add Less Release Resto SOC Ticket';
                        $mail->Body = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new SPK Job Add Less Release Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear '. $department .' Team,'
                                                . 'We would like to inform you that a new SPK Job Add Less Release Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                ...
                // Query untuk memperbarui submit_legal dan catatan_owner di tabel jobadd
                $sql_update_pending = "UPDATE jobadd SET status_jobadd = ?, catatan_jobadd = ?, jobadd_date = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("sssi", $status_jobadd, $catatan_jobadd, $jobadd_date, $id);
                $stmt_update_pending->execute();
                ...
                echo "Status berhasil diperbarui dan data ditahan.";
                    ...
                    //     $sla_fat = date("Y-m-d", strtotime($jobadd_date . ' + ' . $hari_sla_stkonstruksi . ' days'));
                    // } else {
                    //     $conn->rollback();
                    //     echo "Error: Data SLA tidak ditemukan untuk divisi ST-Konstruksi.";
                    //     exit;
                    // }
                    // Jika status tidak diubah menjadi Approve atau Pending, hanya perlu memperbarui status_jobadd
                    $sql_update_other = "UPDATE jobadd SET status_jobadd = ?, catatan_jobadd = ?, jobadd_date = ? WHERE id = ?";
                    $stmt_update_other = $conn->prepare($sql_update_other);
                    $stmt_update_other->bind_param("sssi", $status_jobadd, $catatan_jobadd, $jobadd_date, $id);
        ...
        header("Location: ../datatables-tambahkurang.php");
