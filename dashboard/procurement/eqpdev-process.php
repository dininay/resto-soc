...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_eqpdevprocur"])) {
    ...
    try {
        ...
        $sql_update = "UPDATE equipment SET status_eqpdevprocur = ?, eqpdevprocur_date = ?, status_eqpsite = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("sssi", $status_eqpdevprocur, $eqpdevprocur_date, $status_eqpsite, $id);
        ...
        if ($stmt_update->execute() === TRUE) {
            ...
            if ($status_eqpdevprocur == 'Approve') {
                ...
                $sql_update_kom = "UPDATE equipment SET status_eqpdevprocur = ?, eqpdevprocur_date = ?, status_eqpsite = ? WHERE id = ?";
                $stmt_update_kom = $conn->prepare($sql_update_kom);
                $stmt_update_kom->bind_param("sssi", $status_eqpdevprocur, $eqpdevprocur_date, $status_eqpsite, $id);
