...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["confirm_nego"]) && isset($_POST["catatan_psm"])) {
    ...
    try {
        ...
        if ($confirm_nego == 'Approve') {
            $end_date = date("Y-m-d H:i:s");

            // Query untuk memperbarui status confirm_nego di tabel draft
            $sql_update = "UPDATE draft SET confirm_nego = ?, catatan_psm = ?, end_date = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $confirm_nego, $catatan_psm, $end_date, $id);
            $stmt_update->execute();
