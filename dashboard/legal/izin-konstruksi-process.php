...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_legalizin"])) {
    $id = $_POST["id"];
    $status_legalizin = $_POST["status_legalizin"];
    $legalizin_date = date("Y-m-d H:i:s");
    ...
    try {
        // Query untuk memperbarui status_legalizin dan legalizin_date berdasarkan id
        $sql_update = "UPDATE resto SET status_legalizin = ?, legalizin_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $status_legalizin, $legalizin_date, $id);

        // Eksekusi query update
        if ($stmt_update->execute() === TRUE) {
            // Jika status_spk diubah menjadi Approve
            if ($status_legalizin == 'Approve') {
                // Update status_kom di tabel resto menjadi "In Process"
                $sql_update_kom = "UPDATE resto SET status_legalizin = ?, legalizin_date = ? WHERE id = ?";
                $stmt_update_kom = $conn->prepare($sql_update_kom);
                $stmt_update_kom->bind_param("ssi", $status_legalizin, $legalizin_date, $id);
                ...
                if (!empty($toEmails)) {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Has Been Done Construction Permit by Legal Resto SOC Ticket';
                                        $mail->Body    = '
                                            ...
                                            <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                                ...
                                                <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                    ...
                                                    <p>We would like to inform you that a new Information Has Been Done Construction Permit by Legal Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                        ...
                                        $mail->AltBody = 'Dear SDG-Project Team,'
                                                    . 'We would like to inform you that a new Information Has Been Done Construction Permit by Legal Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            } elseif ($status_legalizin == 'Pending') {
                ...
                $sql_update_re = "UPDATE resto SET status_legalizin = ?, legalizin_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("ssi", $status_legalizin, $legalizin_date, $id);
                ...
                $sql = "UPDATE resto SET status_legalizin = ?, legalizin_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_legalizin, $legalizin_date, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-spk-legal.php");
