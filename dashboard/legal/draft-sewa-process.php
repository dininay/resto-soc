...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["confirm_re"]) && isset($_POST["catatan_re"])) {
    $id = $_POST["id"];
    $confirm_re = $_POST["confirm_re"];
    $catatan_re = $_POST["catatan_re"];
    $draftre_date = null;
        ...
        if ($confirm_re == 'Done') {
            ...
            $queryIR = "SELECT email FROM user WHERE level IN ('Legal')";
                ...
                if (!empty($toEmails)) {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Draft Table Sewa & PSM Has Been Sign by Real Estate Resto SOC Ticket';
                                        $mail->Body    = '
                                            ...
                                            <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                                ...
                                                <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                    ...
                                                    <p>We would like to inform you that a new Information Draft Table Sewa & PSM Has Been Signed by Real Estate Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                        ...
                                        $mail->AltBody = 'Dear Legal Team,'
                                                    . 'We would like to inform you that a new Information Draft Table Sewa & PSM Has Been Signed by Real Estate Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                ...
                // Query untuk memperbarui confirm_re, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE draft SET confirm_re = ?, catatan_re = ?, draftre_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $confirm_re, $catatan_re, $draftre_date, $id);
                ...
                $sql = "UPDATE draft SET confirm_re = ?, catatan_re = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $confirm_re, $catatan_re, $id);
                $stmt->execute();
