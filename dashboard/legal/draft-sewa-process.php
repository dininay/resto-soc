...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["draft_legal"]) && isset($_POST["catatan_draft"])) {
    $id = $_POST["id"];
    $draft_legal = $_POST["draft_legal"];
    $catatan_draft = $_POST["catatan_draft"];
    $start_date = null;
    ...
    try {
        ...
        if ($draft_legal == 'Approve') {
            ...
            $confirm_bod = "In Process";
                ...
                $sql_update_re = "UPDATE draft SET draft_legal = ?, catatan_draft = ?, start_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $draft_legal, $catatan_draft, $start_date, $id);
                ...
                $sql = "UPDATE draft SET draft_legal = ?, catatan_draft = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $draft_legal, $catatan_draft, $id);
                $stmt->execute();
