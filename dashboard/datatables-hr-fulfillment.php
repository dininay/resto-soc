...
Inner join dokumen_loacd ON dokumen_loacd.kode_lahan = socdate_hr.kode_lahan
 WHERE status_ff1 IN ('In Process', 'Pending', 'Done')";
...
$sla_query = "SELECT sla FROM master_slacons WHERE divisi = 'hrga_ff1'";
...
if ($sla_result->num_rows > 0) {
    ...
    echo "No SLA value found for 'hrga_ff1'";
}
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Nama Lahan</th>
                                                <th>Kode Store</th>
                                                <th>Fulfillment Batch 1</th>
                                                <th>Persen Batch 1</th>
                                                <th>Lampiran Fulfillment 1</th>
                                                <th>Status</th>
                                                <th>SLA</th>
												<th>Action</th>
                                            </tr>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <td><?= $row['ff_1'] ?></td>
                                                <td><?= $row['persen_ff1'] ?>%</td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_ff1_files = explode(",", $row['lamp_ff1']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_ff1'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_ff1_files as $ff1) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $ff1 . '" target="_blank">
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['ff1_date'];
                                                    $sla_date = $row['sla_ff1'];
                                                    $status_ff1 = $row['status_ff1'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_ff1'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="hr/hr-ff1-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_ff1'] .'">
                                                            <i class="nav-icon i-Book"></i>
                                                        </button>';
                                                        echo '<a href="datatables-data-ff1.php?id='. $row['kode_lahan'].'" class="btn btn-sm btn-info">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="hr/ff1-process.php" enctype="multipart/form-data">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status FF 1<strong><span style="color: red;">*</span></strong></label>
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_ff1">Catatan FF 1</label>
                                                                        <input type="text" class="form-control" id="catatan_ff1" name="catatan_ff1">
                                                                    </div>
