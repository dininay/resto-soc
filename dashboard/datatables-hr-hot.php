...
WHERE status_hot IN ('In Process', 'Pending', 'Done')";
...
$sla_query = "SELECT sla FROM master_slacons WHERE divisi = 'hot'";
...
if ($sla_result->num_rows > 0) {
    ...
    echo "No SLA value found for 'hot'";
}
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['hot_date'];
                                                    $sla_date = $row['sla_hot'];
                                                    $status_hot = $row['status_hot'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_hot'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="hr/hr-hot-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_hot'] .'">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="hr/hot-process.php" enctype="multipart/form-data">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status HOT<strong><span style="color: red;">*</span></strong></label>
