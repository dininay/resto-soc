...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["onboarding_date"]) && isset($_POST["status_last"])) {
    ...
    try {
        ...
        $sql_get_prev_status = "SELECT status_last, kode_lahan FROM crew2 WHERE id = ?";
        ...
        $sql_update = "UPDATE crew2 SET status_last = ?, acakpt2_date = ?, onboarding_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("sssi", $status_last, $acakpt2_date, $onboarding_date, $id);
        $stmt_update->execute();
        ...
        if ($stmt_update->affected_rows > 0) {
            if ($status_last == 'Done' || $status_last == 'Resign') {
                if ($kode_lahan) {
                    ...
                    // Tambah atau kurangi angka 1 ke kolom crew_onboardkpt2 di tabel socdate_academy
                    if ($prev_status_last == 'Done' && $status_last == 'Resign') {
                        // Decrease crew_onboardkpt2 by 1
                        $sql_update_crew_onboard = "UPDATE socdate_academy SET crew_onboardkpt2 = COALESCE(crew_onboardkpt2, 0) - 1 WHERE kode_lahan = ?";
                        ...
                        // Increase crew_resignkpt2 by 1
                        $sql_update_crew_resign = "UPDATE socdate_academy SET crew_resignkpt2 = COALESCE(crew_resignkpt2, 0) + 1 WHERE kode_lahan = ?";
                        ...
                        $sql_update_crew_onboard = "UPDATE socdate_academy SET crew_onboardkpt2 = COALESCE(crew_onboardkpt2, 0) + 1, crew_allonboardkpt2 = COALESCE(crew_allonboardkpt2, 0) + 1 WHERE kode_lahan = ?";
                        $stmt_update_crew_onboard = $conn->prepare($sql_update_crew_onboard);
                        $stmt_update_crew_onboard->bind_param("s", $kode_lahan);
                        $stmt_update_crew_onboard->execute();
                    }
                    // Calculate mortality_tmaca based on crew_resignkpt2 / crew_allonboardkpt2
                    $sql_calculate_mortality = "UPDATE socdate_academy SET mortality_kpt2 = CASE WHEN crew_allonboardkpt2 > 0 THEN (crew_resignkpt2 / crew_allonboardkpt2) * 100 ELSE 0 END WHERE kode_lahan = ?";
                    ...
                    $sql_get_sla_kpt2 = "SELECT sla_kpt2 FROM socdate_academy WHERE kode_lahan = ?";
                    $stmt_get_sla_kpt2 = $conn->prepare($sql_get_sla_kpt2);
                    $stmt_get_sla_kpt2->bind_param("s", $kode_lahan);
                    $stmt_get_sla_kpt2->execute();
                    $result_get_sla_kpt2 = $stmt_get_sla_kpt2->get_result();
                    $row_sla_kpt2 = $result_get_sla_kpt2->fetch_assoc();
                    $sla_kpt2 = $row_sla_kpt2['sla_kpt2'];

                    // Step 3: Calculate days passed and completion rate
                    $jml_terlewat = 0;
                    if ($sla_kpt2 && strtotime($sla_kpt2) <= strtotime(date('Y-m-d'))) {
                        $jml_terlewat = (strtotime(date('Y-m-d')) - strtotime($sla_kpt2)) / (60 * 60 * 24);
                    }
                    ...
                    $sql_update_comprate = "UPDATE crew2 SET comprate = ? WHERE id = ?";
                    ...
                    $sql_get_comprates = "SELECT comprate FROM crew2 WHERE status_oje = 'Lolos'";
                    ...
                    $sql_update_comprate = "UPDATE socdate_academy SET comprate_kpt2 = ? WHERE kode_lahan = ?";
                    $stmt_update_comprate = $conn->prepare($sql_update_comprate);
        ...
        if (!empty($toEmails)) {
            ...
            try {
                ...
                // Email content
                $mail->Subject = 'Notification: 1 New Information Crew FF 2 Resto SOC Ticket';
                                $mail->Body    = '
                                <div style="font-family: Arial, sans-serif; font-size: 14px; color: #333;">
                                    <div style="background-color: #f7f7f7; padding: 20px; border-radius: 8px;">
                                        ...
                                        <p>We would like to inform you that a new Active Resto SOC Ticket has been created in the Completion Training Rate 2 Process. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
Your prompt attention to this matter is greatly appreciated.</p>
                                ...
                                $mail->AltBody = 'Dear HR Team,'
                                            . 'We would like to inform you that a new Active Resto SOC Ticket has been created in the Completion Training Rate 2 Process. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
Your prompt attention to this matter is greatly appreciated.'
            ...
            echo "No email found for the selected resto or IR users.";
                ...
                $sql_get_kode_lahan = "SELECT kode_lahan FROM crew2 WHERE id = ?";
                ...
                if ($kode_lahan) {
                    $sql_update_re = "UPDATE crew2 SET status_last = ?, acakpt2_date = ? WHERE id = ?";
                    $stmt_update_re = $conn->prepare($sql_update_re);
                    $stmt_update_re->bind_param("ssi", $status_last, $acakpt2_date, $id);
                ...
                $sql = "UPDATE crew2 SET status_last = ?, acakpt2_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_last, $acakpt2_date, $id);
                $stmt->execute();
        ...
        $sql_get_kode = "SELECT kode_lahan FROM crew2 WHERE id = ?";
        ...
        if (isset($kode_lahan) && !empty($kode_lahan)) {
            header("Location: ../datatables-data-kpt2.php?id=" . urlencode($kode_lahan));
            exit;
        } else {
