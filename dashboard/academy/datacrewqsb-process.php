...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["onboarding_date"]) && isset($_POST["status_last"])) {
    ...
    try {
        ...
        $sql_get_prev_status = "SELECT status_last, kode_lahan FROM crewqc WHERE id = ?";
        ...
        $sql_update = "UPDATE crewqc SET status_last = ?, acaqc_date = ?, onboarding_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("sssi", $status_last, $acaqc_date, $onboarding_date, $id);
        $stmt_update->execute();
        ...
        if ($stmt_update->affected_rows > 0) {
            if ($status_last == 'Done' || $status_last == 'Resign') {
                if ($kode_lahan) {
                    ...
                    // Tambah atau kurangi angka 1 ke kolom crew_onboardtmaca di tabel socdate_hraca
                    if ($prev_status_last == 'Done' && $status_last == 'Resign') {
                        // Decrease crew_onboardtmaca by 1
                        $sql_update_crew_onboard = "UPDATE socdate_hraca SET crew_onboardtmaca = COALESCE(crew_onboardtmaca, 0) - 1 WHERE kode_lahan = ?";
                        ...
                        // Increase crew_resigntmaca by 1
                        $sql_update_crew_resign = "UPDATE socdate_hraca SET crew_resigntmaca = COALESCE(crew_resigntmaca, 0) + 1 WHERE kode_lahan = ?";
                        ...
                        $sql_update_crew_onboard = "UPDATE socdate_hraca SET crew_onboardtmaca = COALESCE(crew_onboardtmaca, 0) + 1, crew_allonboardtmaca = COALESCE(crew_allonboardtmaca, 0) + 1 WHERE kode_lahan = ?";
                        $stmt_update_crew_onboard = $conn->prepare($sql_update_crew_onboard);
                        $stmt_update_crew_onboard->bind_param("s", $kode_lahan);
                        $stmt_update_crew_onboard->execute();
                    }
                    // Calculate mortality_tmaca based on crew_resigntmaca / crew_allonboardtmaca
                    $sql_calculate_mortality = "UPDATE socdate_hraca SET mortality_tmaca = CASE WHEN crew_allonboardtmaca > 0 THEN (crew_resigntmaca / crew_allonboardtmaca) * 100 ELSE 0 END WHERE kode_lahan = ?";
                    ...
                    $sql_get_sla_tmaca = "SELECT sla_tmaca FROM socdate_hraca WHERE kode_lahan = ?";
                    $stmt_get_sla_tmaca = $conn->prepare($sql_get_sla_tmaca);
                    $stmt_get_sla_tmaca->bind_param("s", $kode_lahan);
                    $stmt_get_sla_tmaca->execute();
                    $result_get_sla_tmaca = $stmt_get_sla_tmaca->get_result();
                    $row_sla_tmaca = $result_get_sla_tmaca->fetch_assoc();
                    $sla_tmaca = $row_sla_tmaca['sla_tmaca'];

                    // Step 3: Calculate days passed and completion rate
                    $jml_terlewat = 0;
                    if ($sla_tmaca && strtotime($sla_tmaca) <= strtotime(date('Y-m-d'))) {
                        $jml_terlewat = (strtotime(date('Y-m-d')) - strtotime($sla_tmaca)) / (60 * 60 * 24);
                    }
                    ...
                    $sql_update_comprate = "UPDATE crewqc SET comprate = ? WHERE id = ?";
                    ...
                    $sql_get_comprates = "SELECT comprate FROM crewqc WHERE status_oje = 'Lolos'";
                    ...
                    $sql_update_comprate = "UPDATE socdate_hraca SET comprate_tmaca = ? WHERE kode_lahan = ?";
                    $stmt_update_comprate = $conn->prepare($sql_update_comprate);
        ...
        if (!empty($toEmails)) {
            ...
            try {
                ...
                // Email content
                $mail->Subject = 'Notification: 1 New Information Crew QC Resto SOC Ticket';
                                $mail->Body    = '
                                    ...
                                    <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                        ...
                                        <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                            ...
                                            <p>We would like to inform you that a new Information Crew QC Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                ...
                                $mail->AltBody = 'Dear HR Team,'
                                            . 'We would like to inform you that a new Information Crew QC Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "No email found for the selected resto or IR users.";
                ...
                if ($kode_lahan) {
                    $sql_update_re = "UPDATE crewqc SET status_last = ?, acaqc_date = ? WHERE id = ?";
                    $stmt_update_re = $conn->prepare($sql_update_re);
                    $stmt_update_re->bind_param("ssi", $status_last, $acaqc_date, $id);
                ...
                $sql = "UPDATE crewqc SET status_last = ?, acaqc_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_last, $acaqc_date, $id);
                $stmt->execute();
