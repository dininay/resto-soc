...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_kpt2"]) && isset($_POST["catatan_kpt2"]) ) {
    $id = $_POST["id"];
    $status_kpt2 = $_POST["status_kpt2"];
    $catatan_kpt2 = $_POST["catatan_kpt2"];
    $kpt_date2 = null;
    ...
    try {
        ...
        if ($status_kpt2 == 'Done') {
            $kpt_date2 = date("Y-m-d H:i:s");

            // Query untuk memperbarui status status_kpt2 di tabel draft
            $sql_update = "UPDATE socdate_academy SET status_kpt2 = ?, kpt_date2 = ?, catatan_kpt2 = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_kpt2, $kpt_date2, $catatan_kpt2, $id);
            $stmt_update->execute();
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_kpt2, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_academy SET status_kpt2 = ?, kpt_date2 = ?, catatan_kpt2 = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_kpt2, $kpt_date2, $catatan_kpt2, $id);
                ...
                $sql = "UPDATE socdate_academy SET status_kpt2 = ?, kpt_date2 = ?, catatan_kpt2 = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_kpt2, $kpt_date2, $catatan_kpt2, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-hr-kpt-2.php");
