...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_kpt1"]) && isset($_POST["catatan_kpt1"]) ) {
    $id = $_POST["id"];
    $status_kpt1 = $_POST["status_kpt1"];
    $catatan_kpt1 = $_POST["catatan_kpt1"];
    $kpt_date1 = null;
    ...
    try {
        ...
        if ($status_kpt1 == 'Done') {
            $kpt_date1 = date("Y-m-d H:i:s");

            // Query untuk memperbarui status status_kpt1 di tabel draft
            $sql_update = "UPDATE socdate_academy SET status_kpt1 = ?, kpt_date1 = ?, catatan_kpt1 = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_kpt1, $kpt_date1, $catatan_kpt1, $id);
            $stmt_update->execute();
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Crew FF Batch 1 Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Crew FF Batch 1 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear HR Team,'
                                                . 'We would like to inform you that a new Information Crew FF Batch 1 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_kpt1, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_academy SET status_kpt1 = ?, kpt_date1 = ?, catatan_kpt1 = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_kpt1, $kpt_date1, $catatan_kpt1, $id);
                ...
                $sql = "UPDATE socdate_academy SET status_kpt1 = ?, kpt_date1 = ?, catatan_kpt1 = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_kpt1, $kpt_date1, $catatan_kpt1, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-hr-kpt.php");
