...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_kpt3"]) && isset($_POST["catatan_kpt3"]) ) {
    $id = $_POST["id"];
    $status_kpt3 = $_POST["status_kpt3"];
    $catatan_kpt3 = $_POST["catatan_kpt3"];
    $kpt_date3 = null;
    ...
    try {
        ...
        if ($status_kpt3 == 'Done') {
            $kpt_date3 = date("Y-m-d H:i:s");

            // Query untuk memperbarui status status_kpt3 di tabel draft
            $sql_update = "UPDATE socdate_academy SET status_kpt3 = ?, kpt_date3 = ?, catatan_kpt3 = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_kpt3, $kpt_date3, $catatan_kpt3, $id);
            $stmt_update->execute();
            ...
            $sql_get_kode_lahan = "SELECT kode_lahan FROM socdate_academy WHERE id = ?";
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Crew FF Batch 3 Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Crew FF Batch 3 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear HR Team,'
                                                . 'We would like to inform you that a new Information Crew FF Batch 3 Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_kpt3, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_academy SET status_kpt3 = ?, kpt_date3 = ?, catatan_kpt3 = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_kpt3, $kpt_date3, $catatan_kpt3, $id);
                ...
                $sql = "UPDATE socdate_academy SET status_kpt3 = ?, kpt_date3 = ?, catatan_kpt3 = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_kpt3, $kpt_date3, $catatan_kpt3, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-hr-kpt-3.php");
