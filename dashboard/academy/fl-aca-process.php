...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_flaca"]) && isset($_POST["catatan_flaca"]) ) {
    $id = $_POST["id"];
    $status_flaca = $_POST["status_flaca"];
    $catatan_flaca = $_POST["catatan_flaca"];
    $flaca_date = null;
    ...
    try {
        ...
        if ($status_flaca == 'Done') {
            $flaca_date = date("Y-m-d H:i:s");

            // Query untuk memperbarui status status_flaca di tabel draft
            $sql_update = "UPDATE socdate_hraca SET status_flaca = ?, flaca_date = ?, catatan_flaca = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_flaca, $flaca_date, $catatan_flaca, $id);
            $stmt_update->execute();
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Crew FL Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Crew FL Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear HR Team,'
                                                . 'We would like to inform you that a new Information Crew FL Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
            ...
            echo "Status berhasil diperbarui.";
                ...
                // Query untuk memperbarui status_flaca, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_hraca SET status_flaca = ?, flaca_date = ?, catatan_flaca = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_flaca, $flaca_date, $catatan_flaca, $id);
                ...
                $sql = "UPDATE socdate_hraca SET status_flaca = ?, flaca_date = ?, catatan_flaca = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_flaca, $flaca_date, $catatan_flaca, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-aca-fl.php");
