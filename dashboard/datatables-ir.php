...
$sql = "SELECT socdate_ir.*, dokumen_loacd.kode_store, land.nama_lahan, summary_soc.jam_ops
from socdate_ir 
Inner join land ON land.kode_lahan = socdate_ir.kode_lahan
Inner join dokumen_loacd ON dokumen_loacd.kode_lahan = socdate_ir.kode_lahan
LEFT JOIN summary_soc ON summary_soc.kode_lahan = socdate_ir.kode_lahan
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Nama Lahan</th>
                                                <th>Kode Store</th>
                                                <th>Lampiran RAB Cleaning Service</th>
                                                <th>Lampiran RAB Security</th>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                $lamp_rabcs_files = explode(",", $row['lamp_rabcs']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_rabcs'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_rabcs_files as $rabcs) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $rabcs . '" target="_blank">
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_rabsecurity_files = explode(",", $row['lamp_rabsecurity']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_rabsecurity'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_rabsecurity_files as $rabsec) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="../uploads/' . $rabsec . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                            </li>';
                                                    }
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['ir_date'];
                                                    $sla_date = $row['sla_ir'];
                                                    $status_ir = $row['status_ir'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_ir'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="ir/ir-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_ir'] .'">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="ir/ir-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status IR<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_ir" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_ir">Catatan IR</label>
                                                                        <input type="text" class="form-control" id="catatan_ir" name="catatan_ir">
                                                                    </div>
