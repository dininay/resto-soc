                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Kode Store</th>
                                                <th>Nama Lahan</th>
                                                <th>Lampiran WO Req IPAL From SDG Project</th>
                                                <th>Tipe IPAL Yang Dibutuhkan</th>
                                                <th>Note Tipe IPAL</th>
                                                <th>Lampiran WO Rekomendasi IPAL</th>
                                                <th>Lampiran Hasil Baku Mutu</th>
                                                <th>Status</th>
                                                <th>SLA</th>
												<th>Action</th>
                                            </tr>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                ?>
                                                <td><?= $row['tipe_ipal'] ?></td>
                                                <td><?= $row['note_ipalscm'] ?></td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_woipal_files = explode(",", $row['lamp_woipal']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_woipal'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_woipal_files as $listrik) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $listrik . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_hbm_files = explode(",", $row['lamp_hbm']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_hbm'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_hbm_files as $listrik) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $listrik . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    <span class="badge rounded-pill badge-<?php echo $badge_color; ?>">
                                                        <?php echo $row['status_scmipal']; ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <?php
                                                    // Mendapatkan tanggal sla_date dari kolom data
                                                    $slaLegalDate = new DateTime($row['sla_scmipal']);
                                                    
                                                    // Mendapatkan tanggal hari ini
                                                    $today = new DateTime();
                                                    
                                                    // Menghitung selisih hari antara sla_scmipal dan hari ini
                                                    $diff = $today->diff($slaLegalDate);
                                                    
                                                    // Menghitung scoring
                                                    $scoring = calculateScoring($row['scmipal_date'], $row['sla_scmipal'], $sla_value); // Make sure $sla_value is set correctly
                                                    $remarks = getRemarks($scoring);

                                                    if ($row['status_scmipal'] == "Done") {
                                                ...
                                                <td>
                                                <!-- Tombol Edit -->
                                                        <?php if ($row['status_scmipal'] != "Done"): ?>
                                                        <a href="scm/scm-ipal-edit-form.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>
                                                        <?php endif; ?>
                                                        <?php if ($row['status_scmipal'] != "In Process" && $row['status_scmipal'] != "Done"): ?>
                                                            <button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="<?= $row['id'] ?>" data-status="<?= $row['status_scmipal'] ?>">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="scm/scm-ipal-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status SCM IPAL</label>
                                                                        <select class="form-control" id="statusSelect" name="status_scmipal" Placeholder="Pilih">
