...
$sql = "SELECT l.kode_lahan, l.nama_lahan, l.lokasi, l.lamp_land, c.lamp_loacd, d.lamp_draf,
        ...
        p.nama_vendor, p.lamp_vendor, p.lamp_profil, p.lamp_spkrabcons,
...
$sla_query = "SELECT sla FROM master_sla WHERE divisi = 'KOM'";
...
if ($sla_result->num_rows > 0) {
    ...
    echo "No SLA value found for 'KOM'";
}
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                if (!empty($row['start_konstruksi'])) {
                                                    // Format tanggal jika data ada
                                                    $date = new DateTime($row['start_konstruksi']);
                                                    $formattedDate = $date->format('d M y');
                                                } else {
                                                ...
                                                <td>
                                                    ...
                                                    ?>
                                                    <span class="badge rounded-pill badge-<?php echo $badge_color; ?>">
                                                        <?php echo $row['obstacle_kom']; ?>
                                                    </span>
                                                </td>
                                                <td><?= $row['note_kom'] ?></td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_obskom_files = explode(",", $row['lamp_obskom']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_obskom'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_obskom_files as $kom) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $kom . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['kom_date'];
                                                    $sla_date = $row['sla_kom'];
                                                    $status_kom = $row['status_kom'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_kom'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        ...
                                                        echo '<button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_kom'] .'">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="sdg-pk/kom-sdgpk-process.php" enctype="multipart/form-data">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status Approve KOM<strong><span style="color: red;">*</span></strong></label>
