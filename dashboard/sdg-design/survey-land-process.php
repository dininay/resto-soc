...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_survey"])) {
    $id = $_POST["id"];
    ...
    try {
        // Query untuk memperbarui status_survey berdasarkan id
        $sql_update_sdg_desain = "UPDATE sdg_desain SET status_survey = ?, survey_date = ? WHERE id = ?";
        $stmt_update_sdg_desain = $conn->prepare($sql_update_sdg_desain);
        $stmt_update_sdg_desain->bind_param("ssi", $status_survey, $survey_date, $id);

        // Eksekusi query
        if ($stmt_update_sdg_desain->execute() === TRUE) {
            // Jika status_survey diubah menjadi Approve
            if ($status_survey == 'Done') {
                // Ambil data dari tabel sdg_desain berdasarkan id yang diedit
                $sql_select = "SELECT kode_lahan, survey_date, lamp_desainplan FROM sdg_desain WHERE id = ?";
                ...
                if ($row = $result_select->fetch_assoc()) {
                    ...
                    $sql_select_sla_qs = "SELECT sla FROM master_sla WHERE divisi = 'Layouting'";
                    $result_sla_qs = $conn->query($sql_select_sla_qs);
                    
                    if ($row_sla_qs = $result_sla_qs->fetch_assoc()) {
                        $sla_days_qs = $row_sla_qs['sla'];
                        $survey_date_obj = new DateTime($row['survey_date']);
                        $survey_date_obj->modify("+$sla_days_qs days");
                        $sla_date = $survey_date_obj->format("Y-m-d");

                        $status_obssdg = "In Process";

                        // Masukkan data ke tabel sdg_rab
                        $sql_update_re = "UPDATE sdg_desain SET sla_layout = ?, status_obssdg = ? WHERE id = ?";
                        $stmt_update_re = $conn->prepare($sql_update_re);
                        $stmt_update_re->bind_param("ssi", $sla_date, $status_obssdg, $id);
                        $stmt_update_re->execute();
                    } else {
                        $conn->rollback();
                        echo "Error: SLA not found for divisi: Layout.";
                        exit;
                    }
                ...
                // Query untuk memperbarui status_survey, survey_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE sdg_desain SET status_survey = ?, survey_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("ssi", $status_survey, $survey_date, $id);
                $stmt_update_re->execute();
                ...
                // Jika status tidak diubah menjadi Approve, Reject, atau Pending, hanya perlu memperbarui status_survey di tabel re
                $sql = "UPDATE sdg_desain SET status_survey = ?, survey_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_survey, $survey_date, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-land-survey.php");
