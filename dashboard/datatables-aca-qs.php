                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                $lamp_tm_files = explode(",", $row['lamp_tm']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_tm'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_tm_files as $tm) {
                                                ...
                                                ?>
                                                <td><?= $row['crew_neededtmaca'] ?></td>
                                                <td><?= $row['crew_onboardtmaca'] ?></td>
                                                <td><?= intval($row['mortality_tmaca']) ?>%</td>
                                                <td><?= intval($row['comprate_tmaca']) ?>%</td>
                                                <td>
                                                    <?php
                                                        // Tentukan warna badge berdasarkan status approval owner
                                                        $badge_color = '';
                                                        switch ($row['status_tmaca']) {
                                                            case 'Done':
                                                                $badge_color = 'success';
                                                                break;
                                                            case 'Pending':
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['tmaca_date'];
                                                    $sla_date = $row['sla_tmaca'];
                                                    $status_tmaca = $row['status_tmaca'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_tmaca'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="academy/aca-tm-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_tmaca'] .'">
                                                            <i class="nav-icon i-Book"></i>
                                                        </button>';
                                                        echo '<a href="datatables-data-acaqs.php?id='. $row['kode_lahan'].'" class="btn btn-sm btn-info">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="academy/tm-aca-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status QC<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_tmaca" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_tmaca">Catatan QC</label>
