...
// $sql = "SELECT socdate_academy.*, socdate_hr.fl, socdate_hr.lamp_fl, socdate_hr.status_flaca, socdate_hr.flaca_date, dokumen_loacd.kode_store, land.nama_lahan, socdate_hr.sla_fl
// from socdate_hr
// Inner join land ON land.kode_lahan = socdate_hr.kode_lahan
// left join socdate_academy ON socdate_academy.kode_lahan = socdate_hr.kode_lahan
// Inner join dokumen_loacd ON dokumen_loacd.kode_lahan = socdate_hr.kode_lahan
// -- WHERE status_fl = 'Done'";
$sql = "SELECT socdate_hr.*, socdate_hraca.*, dokumen_loacd.kode_store, land.nama_lahan 
from socdate_hr 
Inner join land ON land.kode_lahan = socdate_hr.kode_lahan
Inner join socdate_hraca ON socdate_hraca.kode_lahan = socdate_hr.kode_lahan
Inner join dokumen_loacd ON dokumen_loacd.kode_lahan = socdate_hr.kode_lahan
-- WHERE status_fl = 'Done'";
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                $lamp_fl_files = explode(",", $row['lamp_fl']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_fl'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_fl_files as $tm) {
                                                ...
                                                ?>
                                                <td><?= $row['crew_neededflaca'] ?></td>
                                                <td><?= $row['crew_onboardflaca'] ?></td>
                                                <td><?= intval($row['mortality_flaca']) ?>%</td>
                                                <td><?= intval($row['comprate_flaca']) ?>%</td>
                                                <td>
                                                    <?php
                                                        // Tentukan warna badge berdasarkan status approval owner
                                                        $badge_color = '';
                                                        switch ($row['status_flaca']) {
                                                            case 'Done':
                                                                $badge_color = 'success';
                                                                break;
                                                            case 'Pending':
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['flaca_date'];
                                                    $sla_date = $row['sla_flaca'];
                                                    $status_flaca = $row['status_flaca'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_flaca'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="academy/aca-fl-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_flaca'] .'">
                                                            <i class="nav-icon i-Book"></i>
                                                        </button>';
                                                        echo '<a href="datatables-data-acafl.php?id='. $row['kode_lahan'].'" class="btn btn-sm btn-info">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="academy/fl-aca-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status FL<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_flaca" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_tmaca">Catatan FL</label>
