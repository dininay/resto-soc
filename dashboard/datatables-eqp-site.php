...
if ($sla_result->num_rows > 0) {
    ...
    echo "No SLA value found for 'Sign'";
}
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Lampiran EQP Delivery</th>
                                                <th>Lampiran SPK Equipment</th>
                                                <th>Lampiran EQP On Site</th>
                                                <th>Progress EQP On Site</th>
                                                <th>Defect</th>
                                                <th>Note Defect</th>
                                                <th>Status EQP On Site</th>
                                                <th>SLA</th>
												<th>Action</th>
                                            </tr>
                                        </thead>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_eqpdev_files = explode(",", $row['lamp_eqpdev']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_eqpdev'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_eqpdev_files as $ba) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $ba . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_eqpsite_files = explode(",", $row['lamp_eqpsite']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_eqpsite'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_eqpsite_files as $ba) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $ba . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                ?>       
                                                <td><?= $row['progress_eqpsite'] ?></td>
                                                <td><?= $row['defect_eqpsite'] ?></td>
                                                <td><?= $row['notedefect_eqpsite'] ?></td>
                                                
                                                <td>
                                                    <?php
                                                        // Tentukan warna badge berdasarkan status approval owner
                                                        $badge_color = '';
                                                        switch ($row['status_eqpsite']) {
                                                            case 'Done':
                                                                $badge_color = 'success';
                                                                break;
                                                            case 'Pending':
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['eqpsite_date'];
                                                    $sla_date = $row['sla_eqpsite'];
                                                    $status_eqpsite = $row['status_eqpsite'];
                                                ...
                                                <td>
                                                    <!-- Tombol Edit -->
                                                    <?php if ($row['status_eqpsite'] != "Approve"): ?>
                                                    <?php
                                                    // Mengatur timezone ke Asia/Jakarta (sesuaikan dengan timezone lokal Anda)
                                                    date_default_timezone_set('Asia/Jakarta');
                                                    ...
                                                    $sla_steqp = new DateTime($row['sla_steqp']);
                                                    // Menghitung selisih hari
                                                    $interval = $today->diff($sla_steqp)->format("%r%a");

                                                    if ($row['status_eqpsite'] != "Approve" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="sdg-pk/eqpsite-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_eqpsite'] .'">
                                                        ...
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    <form id="statusForm" method="post" action="sdg-pk/eqpsite-process.php"  enctype="multipart/form-data">
                                                                        <input type="hidden" name="id" id="modalId" value="<?= $row['id']; ?>">
                                                                        <div class="form-group">
                                                                            <label for="statusSelect">Status EQP On Site<strong><span style="color: red;">*</span></strong></label>
                                                                            <select class="form-control" id="statusSelect" name="status_eqpsite">
