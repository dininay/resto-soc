...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_land"])) {
    ...
    try {
        ...
        if ($status_land == 'Approve' || $status_land == 'In Process'|| $status_land == 'Pending') {
        ...
        header("Location: datatables-resto-name.php");
...
$sql = "SELECT l.kode_lahan, l.nama_lahan, l.lokasi, l.nama_lahan, r.status_land, r.id, dl.kode_store
        FROM land l
        INNER JOIN resto r ON l.kode_lahan = r.kode_lahan
        INNER JOIN dokumen_loacd dl ON l.kode_lahan = dl.kode_lahan
        WHERE dl.status_approvlegalvd = 'Approve'";
