    ...
    try {
        ...
        if ($status_approvnego == 'Approve') {
            ...
            if ($stmt->execute() === TRUE) {
                ...
                $sql_sdgd = "INSERT INTO sdg_desain (kode_lahan, confirm_sdgdesain, status_survey, sla_survey) 
                             VALUES (?, ?, ?, ?)";
                $stmt_sdgd = $conn->prepare($sql_sdgd);
                $stmt_sdgd->bind_param("ssss", $updated_row['kode_lahan'], $confirm_sdgdesain, $status_survey, $sla_survey);
                $stmt_sdgd->execute();
