...
$sql = "SELECT socdate_hr.*, dokumen_loacd.kode_store, land.nama_lahan 
from socdate_hr 
...
WHERE status_ff2 IN ('In Process', 'Pending', 'Done')";
...
$sla_query = "SELECT sla FROM master_slacons WHERE divisi = 'hrga_ff2'";
...
if ($sla_result->num_rows > 0) {
    ...
    echo "No SLA value found for 'hrga_ff2'";
}
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Nama Lahan</th>
                                                <th>Kode Store</th>
                                                <th>Fulfillment Batch 2</th>
                                                <th>Persen Batch 2</th>
                                                <th>Lampiran Fulfillment 2</th>
                                                <th>Status</th>
                                                <th>SLA</th>
												<th>Action</th>
                                            </tr>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <td><?= $row['ff_2'] ?></td>
                                                <td><?= $row['persen_ff2'] ?>%</td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_ff2_files = explode(",", $row['lamp_ff2']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_ff2'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_ff2_files as $ff2) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $ff2 . '" target="_blank">
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['ff2_date'];
                                                    $sla_date = $row['sla_ff2'];
                                                    $status_ff2 = $row['status_ff2'];
                                                ...
                                                <td>
                                                    ...
                                                    if ($row['status_ff2'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="hr/hr-ff2-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_ff2'] .'">
                                                            <i class="nav-icon i-Book"></i>
                                                        </button>';
                                                        echo '<a href="datatables-data-ff2.php?id='. $row['kode_lahan'].'" class="btn btn-sm btn-info">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="hr/ff2-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status FF 2<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_ff2" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_ff2">Catatan FF 2</label>
                                                                        <input type="text" class="form-control" id="catatan_ff2" name="catatan_ff2">
                                                                    </div>
