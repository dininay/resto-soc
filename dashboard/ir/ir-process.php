...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_ir"]) && isset($_POST["catatan_ir"]) ) {
    $id = $_POST["id"];
    $status_ir = $_POST["status_ir"];
    $catatan_ir = $_POST["catatan_ir"];
    $ir_date = null;
    ...
    try {
        ...
        if ($status_ir == 'Approve') {
            $ir_date = date("Y-m-d H:i:s");

            // Query untuk memperbarui status status_ir di tabel draft
            $sql_update = "UPDATE socdate_ir SET status_ir = ?, ir_date = ?, catatan_ir = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("sssi", $status_ir, $ir_date, $catatan_ir, $id);
            $stmt_update->execute();
            ...
            if (!empty($toEmails)) {
                ...
                try {
                    ...
                    // Email content
                    $mail->Subject = 'Notification: 1 New Information Prepare RTO by HR (CS & Security) Resto SOC Ticket';
                                    $mail->Body    = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Information Prepare RTO by HR (CS & Security) Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear PMO Team,'
                                                . 'We would like to inform you that a new Information Prepare RTO by HR (CS & Security) Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                ...
                // Query untuk memperbarui status_ir, vl_date di tabel re dan memasukkan data ke dalam tabel hold_project
                $sql_update_re = "UPDATE socdate_ir SET status_ir = ?, ir_date = ?, catatan_ir = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_ir, $ir_date, $catatan_ir, $id);
                ...
                $sql = "UPDATE socdate_ir SET status_ir = ?, ir_date = ?, catatan_ir = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_ir, $ir_date, $catatan_ir, $id);
                $stmt->execute();
