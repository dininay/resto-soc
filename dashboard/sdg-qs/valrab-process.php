...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["confirm_sdgqs"]) && isset($_POST["catatan_sdgqs"])) {
    ...
    try {
        ...
        if ($stmt_update->execute() === TRUE) {
            // Jika confirm_sdgqs diubah menjadi Done
                ...
                if ($row = $result_select->fetch_assoc()) {
                    ...
                    if ($row_sla = $result_sla->fetch_assoc()) {
                        ...
                        if ($stmt_check->num_rows > 0) {
                            ...
                            $sql_update = "UPDATE procurement SET status_approvsdg = ?, status_tender = ?, sla_date = ? 
                                        WHERE kode_lahan = ?";
                            $stmt_update = $conn->prepare($sql_update);
                            $status_approvprocurement = "In Process";
                            $status_tender = "In Process";
                            $stmt_update->bind_param("ssss", $confirm_sdgqs, $status_tender, $sla_date, $row['kode_lahan']);
                            $stmt_update->execute();
                        } else {
                            // Jika tidak ada, lakukan insert
                            $sql_insert = "INSERT INTO procurement (kode_lahan, status_approvsdg, status_tender, sla_date) 
                                        VALUES (?, ?, ?, ?)";
                            $stmt_insert = $conn->prepare($sql_insert);
                            $status_approvprocurement = "In Process";
                            $status_tender = "In Process";
                            $stmt_insert->bind_param("ssss", $row['kode_lahan'], $confirm_sdgqs, $status_tender, $sla_date);
                            $stmt_insert->execute();
                        }
                ...
                foreach ($departments as $department) {
                    ...
                    if (!empty($toEmails)) {
                        try {
                            ...
                            $mail->Subject = 'Notification: New Active SPK Construction Resto SOC Ticket';
                            $mail->Body = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new Active SPK Construction Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear '. $department .' Team,'
                                                . 'We would like to inform you that a new Active SPK Construction Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                ...
                foreach ($departments as $department) {
                    ...
                    if (!empty($toEmails)) {
                        try {
                            ...
                            // Email content with personalized greeting
                            $mail->Subject = 'Notification: New Active Design Construction Revision Resto SOC Ticket';
                            $mail->Body = '
                                        <div style="font-family: Arial, sans-serif; font-size: 14px; color: #333; margin: 0; padding: 0;">
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            <img src="cid:embedded_image" alt="Header Image" style="display: block; width: 50%; height: auto; margin: 0 auto;">
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                <h2 style="font-size: 20px; color: #5cb85c; margin-bottom: 10px;">Dear '. $department .' Team,</h2>
                                                <p>We would like to inform you that a new Active Design Construction Revision Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                                Your prompt attention to this matter is greatly appreciated.</p>
                                                <p></p>
                                                <p>Have a good day!</p>
                                            </div>
                                    ...
                                    $mail->AltBody = 'Dear '. $department .' Team,'
                                                . 'We would like to inform you that a new Active Design Construction Revision Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                                Your prompt attention to this matter is greatly appreciated.'
                                                . 'Have a good day!';
