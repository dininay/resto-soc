...
WHERE p.status_spkfat IN ('Approve','Done Review')
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Lampiran SPK Done Paraf</th>
                                                <th>Status TAF</th>
                                                <th>Catatan TAF</th>
                                                <th>Lampiran Final SPK</th>
                                                <th>Status Final SPK</th>
                                                <th>SLA</th>
                                                <th>Action</th>
                                            </tr>
                                        </thead>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_spkconsdone_files = explode(",", $row['lamp_spkconsdone']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_spkconsdone'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_spkconsdone_files as $lamp_vd) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $lamp_vd . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    ?>
                                                    <span class="badge rounded-pill badge-<?php echo $badge_color; ?>">
                                                        <?php echo $row['status_spkfat']; ?>
                                                    </span>
                                                </td>
                                                <td><?= $row['catatan_spkfat'] ?></td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_finalspk_files = explode(",", $row['lamp_finalspk']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_finalspk'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_finalspk_files as $lamp_vd) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $lamp_vd . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['finalspk_date'];
                                                    $sla_date = $row['sla_finalspk'];
                                                    $status_finalspk = $row['status_finalspk'];
                                                ...
                                                <td>
                                                    <!-- Tombol Edit -->
                                                    <?php if ($row['status_finalspk'] != "Done"): ?>
                                                    <?php
                                                    // Mengatur timezone ke Asia/Jakarta (sesuaikan dengan timezone lokal Anda)
                                                    date_default_timezone_set('Asia/Jakarta');
                                                    ...
                                                    if ($row['status_finalspk'] != "Done") {
                                                        echo '<a href="procurement/final-spk-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_finalspk'] .'">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="procurement/final-spk-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status Final SPK<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_finalspk" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_finalspk">Catatan Procurement</label>
                                                                        <input type="text" class="form-control" id="catatan_finalspk" name="catatan_finalspk">
                                                                    </div>
