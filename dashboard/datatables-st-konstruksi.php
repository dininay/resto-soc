...
$sla_query = "SELECT sla FROM master_sla WHERE divisi = 'ST-Konstruksi'";
		...
		?>
            ...
            <div class="main-content">
                <div class="breadcrumb">
                    <h1>List Data ST Construction Activity</h1>
                </div>
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                if (!empty($row['lamp_spkrabcons'])) {
                                                    ...
                                                    foreach ($lamp_spkrabcons_files as $spk) {
                                                ...
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_kom_files = explode(",", $row['lamp_stkonstruksi']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                ...
                                                <td>
                                                    ...
                                                    <span class="badge rounded-pill badge-<?php echo $badge_color; ?>">
                                                        <?php echo $row['obstacle_stkons']; ?>
                                                    </span>
                                                </td>      
                                                <td><?= $row['note_stkons'] ?></td>        
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_obsstkons_files = explode(",", $row['lamp_obsstkons']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_obsstkons'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_obsstkons_files as $kom) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $kom . '" target="_blank">
                                                                    <i class="fas fa-file-pdf nav-icon"></i>
                                                                </a>
                                                ...
                                                <td>
                                                    ...
                                                    $slaLegalDate = new DateTime($row['sla_stkonstruksi']);
                                                    
                                                    // Mendapatkan tanggal hari ini
                                                    $today = new DateTime();
                                                    
                                                    // Menghitung selisih hari antara sla_stkonstruksi dan hari ini
                                                    $diff = $today->diff($slaLegalDate);
                                                    
                                                    // Menghitung scoring
                                                    $scoring = calculateScoring($row['stkonstruksi_date'], $row['sla_stkonstruksi'], $sla_value); // Make sure $sla_value is set correctly
                                                    $remarks = getRemarks($scoring);

                                                    if ($row['status_stkonstruksi'] == "Approve") {
                                                ...
                                                <td>
                                                    ...
                                                    $sla_stkonstruksi = new DateTime($row['sla_stkonstruksi']);
                                                    // Menghitung selisih hari
                                                    $interval = $today->diff($sla_stkonstruksi)->format("%r%a");

                                                    // Menampilkan tombol jika status bukan Approve dan sudah mendekati H-30 dari deadline
                                                    if ($row['status_stkonstruksi'] !== 'Approve' && $interval <= 30 && $interval >= 0) : ?>
                                                        <div>
                                                            <a href="sdg-pk/stkons-edit-form.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning mb-2">
                                                                <i class="i-Pen-2"></i>
                                                            </a>
                                                            <button class="btn btn-sm btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="<?= $row['id'] ?>" data-status="<?= $row['status_stkonstruksi'] ?>">
                                                        ...
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    <form id="statusForm" method="post" action="sdg-pk/stkons-process.php"  enctype="multipart/form-data">
                                                                        <input type="hidden" name="id" id="modalId" value="<?= $row['id']; ?>">
                                                                        <div class="form-group">
                                                                            <label for="statusSelect">Status Approve ST Kontraktor</label>
                                                                            <select class="form-control" id="statusSelect" name="status_stkonstruksi">
...
<?php if ($sla_stkonstruksi == 'Pending') { ?>
    <script>
        $(document).ready(function () {
