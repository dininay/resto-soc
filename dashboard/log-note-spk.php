...
// Query untuk mengambil data dari tabel note_spk
...
if ($result->num_rows > 0) {
    ...
    $catatan_proc = $row['catatan_proc'];
    $proc_date = $row['proc_date'];
    $catatan_spkfat = $row['catatan_spkfat'];
    $taf_date = $row['taf_date'];
} else {
    $catatan_proc = '';
    $proc_date = '';
    $catatan_spkfat = '';
        ...
        <div class="main-content-wrap d-flex flex-column">
            ...
            <div class="main-content">
                <div class="breadcrumb">
                    <h1>History Note SPK Construction (TAF - Procurement)</h1>
                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <div class="chat-container">
                                        <?php if (!empty($catatan_proc)): ?>
                                            <div class="chat-bubble chat-left">
                                                <div class="chat-date"><?php echo date('M d, Y', strtotime($proc_date)); ?> | Catatan Procurement</div>
                                                <div><?php echo nl2br(htmlspecialchars($catatan_proc)); ?></div>
                                            </div>
                                        <?php endif; ?>

                                        <?php if (!empty($catatan_spkfat)): ?>
                                            <div class="chat-bubble chat-right">
                                                <div class="chat-date"><?php echo date('M d, Y', strtotime($taf_date)); ?> | Catatan TAF</div>
                                                <div><?php echo nl2br(htmlspecialchars($catatan_spkfat)); ?></div>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
...
<!-- ============ Search UI Start ============= -->
    ...
    <script src="../dist-assets/js/scripts/datatables.script.min.js"></script>	<script src="../dist-assets/js/icons/feather-icon/feather.min.js"></script>
