...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_stkonstruksi"])) {
    $id = $_POST["id"];
    $status_stkonstruksi = $_POST["status_stkonstruksi"];
    $stkonstruksi_date = date("Y-m-d H:i:s");
    ...
    try {
        ...
        $sql_update = "UPDATE resto SET status_stkonstruksi = ?, stkonstruksi_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $status_stkonstruksi, $stkonstruksi_date, $id);

        // Eksekusi query update
        if ($stmt_update->execute() === TRUE) {
            // Jika status_finallegal diubah menjadi Approve
            if ($status_stkonstruksi == 'Approve') {
                // Update status_kom di tabel resto menjadi "In Process"
                $sql_update_kom = "UPDATE resto SET status_stkonstruksi = ?, stkonstruksi_date = ? WHERE id = ?";
                $stmt_update_kom = $conn->prepare($sql_update_kom);
                $stmt_update_kom->bind_param("ssi", $status_stkonstruksi, $stkonstruksi_date, $id);
                ...
                foreach ($departments as $department) {
                    ...
                    if (!empty($toEmails)) {
                        try {
                            ...
                            $mail->Subject = 'Notification: New ST Contractor Has Been Done by SDG-Project Resto SOC Ticket';
                            $mail->Body = '
                                        ...
                                        <div style="background-color: #f7f7f7; border-radius: 8px; padding: 0; margin: 0; text-align: center;">
                                            ...
                                            <div style="padding: 20px; background-color: #f7f7f7; border-radius: 8px;">
                                                ...
                                                <p>We would like to inform you that a new ST Contractor Has Been Done by SDG-Project Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                                    ...
                                    $mail->AltBody = 'Dear '. $department .' Team,'
                                                . 'We would like to inform you that a new ST Contractor Has Been Done by SDG-Project Resto SOC Ticket has been created. This needs your attention, please log in to the SOC application to review the details at your earliest convenience.
                ...
                $sql_update_re = "UPDATE resto SET status_stkonstruksi = ?, stkonstruksi_date = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("ssi", $status_stkonstruksi, $stkonstruksi_date, $id);
                ...
                $sql = "UPDATE resto SET status_stkonstruksi = ?, stkonstruksi_date = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssi", $status_stkonstruksi, $stkonstruksi_date, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-st-konstruksi.php");
