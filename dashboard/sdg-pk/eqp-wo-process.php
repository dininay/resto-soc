...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_woeqp"])) {
    $id = $_POST["id"];
    $status_woeqp = $_POST["status_woeqp"];
    ...
    try {
        $status_eqpdevprocur = "In Process";
        // Query untuk memperbarui status_finallegal berdasarkan id
        $sql_update = "UPDATE equipment SET status_woeqp = ?, eqpdev_date = ?, status_eqpdevprocur = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("sssi", $status_woeqp, $eqpdev_date, $status_eqpdevprocur, $id);

        // Eksekusi query update
        if ($stmt_update->execute() === TRUE) {
            // Jika status_finallegal diubah menjadi Approve
            if ($status_woeqp == 'Proceed By Procurement') {
                // Update status_kom di tabel resto menjadi "In Process"
                $sql_update_kom = "UPDATE equipment SET status_woeqp = ?, eqpdev_date = ?, status_eqpdevprocur = ? WHERE id = ?";
                $stmt_update_kom = $conn->prepare($sql_update_kom);
                $stmt_update_kom->bind_param("sssi", $status_woeqp, $eqpdev_date, $status_eqpdevprocur, $id);
            ...
            foreach ($departments as $department) {
                ...
                $sql_update_re = "UPDATE equipment SET status_woeqp = ?, eqpdev_date = ?, status_eqpdevprocur = ? WHERE id = ?";
                $stmt_update_re = $conn->prepare($sql_update_re);
                $stmt_update_re->bind_param("sssi", $status_woeqp, $eqpdev_date, $status_eqpdevprocur, $id);
                ...
                $sql = "UPDATE equipment SET status_woeqp = ?, eqpdev_date = ?, status_eqpdevprocur = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sssi", $status_woeqp, $eqpdev_date, $status_eqpdevprocur, $id);
                $stmt->execute();
        ...
        header("Location: ../datatables-wo-eqp.php");
