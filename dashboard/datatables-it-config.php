...
$sql = "SELECT socdate_it.*, dokumen_loacd.kode_store, land.nama_lahan 
from socdate_it 
Inner join land ON land.kode_lahan = socdate_it.kode_lahan
Inner join dokumen_loacd ON dokumen_loacd.kode_lahan = socdate_it.kode_lahan";
                        ...
                        <div class="card text-left">
                            <div class="card-body">
								...
								<div class="footer-bottom float-right">
									<!-- <p><a class="btn btn-primary btn-icon m-1" href="it/itconfig-from.php">+ add Data </a></p> -->
									<p>
                                    ...
                                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                                        ...
                                        <?php foreach ($data as $row): ?>
                                            <tr>
                                                ...
                                                <td><?= $row['kode_dvr'] ?></td>
                                                <td><?= $row['web_report'] ?></td>
                                                <td><?= $row['akun_gis'] ?></td>
                                                <?php
                                                // Bagian ini di dalam loop yang menampilkan data tabel
                                                $lamp_config_files = explode(",", $row['lamp_config']); // Pisahkan nama file menjadi array
                                                // Periksa apakah array tidak kosong sebelum menampilkan ikon
                                                if (!empty($row['lamp_config'])) {
                                                    echo '<td>
                                                            <ul style="list-style-type: none; padding: 0; margin: 0;">';
                                                    // Loop untuk setiap file dalam array
                                                    foreach ($lamp_config_files as $config) {
                                                        echo '<li style="display: inline-block; margin-right: 5px;">
                                                                <a href="uploads/' . $config . '" target="_blank">
                                                ...
                                                <td>
                                                    ...
                                                    $start_date = $row['config_date'];
                                                    $sla_date = $row['sla_itconfig'];
                                                    $status_itconfig = $row['status_itconfig'];
                                                ...
                                                <td>
                                                    <!-- Tombol Edit -->
                                                    <?php if ($row['status_itconfig'] != "Done"): ?>
                                                    <?php
                                                    // Mengatur timezone ke Asia/Jakarta
                                                    date_default_timezone_set('Asia/Jakarta');
                                                    ...
                                                    if ($row['status_itconfig'] != "Done" && $current_time >= $work_start && $current_time <= $work_end) {
                                                        echo '<a href="it/itconfig-edit-form.php?id='. $row['id'] .'" class="btn btn-sm btn-warning mr-2">
                                                            <i class="nav-icon i-Pen-2"></i>
                                                        </a>';
                                                        echo '<button class="btn btn-sm btn-primary edit-btn mr-2" data-toggle="modal" data-target="#editModal" data-id="'. $row['id'] .'" data-status="'.$row['status_itconfig'] .'">
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="statusForm" method="post" action="it/it-config-process.php" enctype="multipart/form-data">
                                                                    <input type="hidden" name="id" value=<?= $row['id'] ?> id="modalKodeLahan">
                                                                    <div class="form-group">
                                                                        <label for="statusSelect">Status IT<strong><span style="color: red;">*</span></strong></label>
                                                                        <select class="form-control" id="statusSelect" name="status_itconfig" Placeholder="Pilih">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <label for="catatan_config">Catatan IT Config</label>
                                                                        <input type="text" class="form-control" id="catatan_config" name="catatan_config">
                                                                    </div>
...
<?php if ($status_itconfig == 'Pending') { ?>
    <script>
        $(document).ready(function () {
