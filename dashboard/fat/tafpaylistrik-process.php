...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["status_tafpaylistrik"])) {
    $id = $_POST["id"];
    $status_tafpaylistrik = $_POST["status_tafpaylistrik"];
    var_dump($id); // Debugging untuk memastikan ID diterima
    var_dump($status_tafpaylistrik); // Debugging untuk memastikan status diterima
    $tafpaylistrik_date = null;
    ...
    try {
        $tafpaylistrik_date = date("Y-m-d H:i:s");

        // Jika status_tafpaylistrik diubah menjadi Proceed
        if ($status_tafpaylistrik == 'Paid') {
            var_dump($status_tafpaylistrik); // Debugging untuk memastikan status Proceed masuk

            $status_sdglistrik = "Done";
            // Query untuk memperbarui status_tafpaylistrik dan obstacle
            $sql = "UPDATE socdate_sdg SET status_tafpaylistrik = ?, tafpaylistrik_date = ?, status_sdglistrik = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssi", $status_tafpaylistrik, $tafpaylistrik_date, $status_sdglistrik, $id);
        ...
        } elseif ($status_tafpaylistrik == 'Pending') {
            // Ambil kode_lahan dari tabel socdate_sdg
            $sql_get_kode_lahan = "SELECT kode_lahan FROM socdate_sdg WHERE id = ?";
            $stmt_get_kode_lahan = $conn->prepare($sql_get_kode_lahan);
            $stmt_get_kode_lahan->bind_param("i", $id);
            ...
            $sql = "UPDATE socdate_sdg SET status_tafpaylistrik = ?, tafpaylistrik_date = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssi", $status_tafpaylistrik, $tafpaylistrik_date, $id);
            ...
            // Jika status tidak diubah menjadi Approve, Reject, atau Pending, hanya perlu memperbarui status_tafpaylistrik
            $sql = "UPDATE socdate_sdg SET status_tafpaylistrik = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $status_tafpaylistrik, $id);
