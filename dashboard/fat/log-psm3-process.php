...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["kode_lahan"])) {
    ...
    $fat3_date = date("Y-m-d H:i:s");

    if ($_POST['review_fat3'] == "Done") {
        $review_fat3 = "Done";
    } else {
        $review_fat3 = isset($_POST['manual_note2']) ? $_POST['manual_note2'] : "";
    }

    $sql_update = "UPDATE note_psm SET review_fat3 = ?, fat3_date = ? WHERE id = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("ssi", $review_fat3, $fat3_date, $id);
    $stmt_update->execute();
    // Mulai transaksi
