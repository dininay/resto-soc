    ...
    try {
        ...
        if ($stmt_update->execute() === TRUE) {
            ...
            if ($status_eqptaf == 'Approve') {
                ...
                $sql_update_pending = "UPDATE equipment SET status_eqptaf = ?, catatan_eqptaf = ?, eqptaf_date = ?, status_woeqp = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("ssssi", $status_eqptaf, $catatan_eqptaf, $eqptaf_date, $status_woeqp, $id);
