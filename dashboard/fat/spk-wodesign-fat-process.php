...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])  && isset($_POST["status_spkwofat"])&& isset($_POST["catatan_spkwofat"])) {
    $id = $_POST["id"];
    $status_spkwofat = $_POST["status_spkwofat"];
    $catatan_spkwofat = $_POST["catatan_spkwofat"];
    ...
    try {
        // Query untuk memperbarui status_spkwofat berdasarkan id
        $sql_update = "UPDATE sdg_desain SET status_spkwofat = ?, catatan_spkwofat = ?, spkwofat_date = ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $spkwofat_date = date("Y-m-d");
        $stmt_update->bind_param("sssi", $status_spkwofat, $catatan_spkwofat, $spkwofat_date, $id);

        // Eksekusi query update
        if ($stmt_update->execute() === TRUE) {
            // Jika status_spkwofat diubah menjadi 'Approve'
            if ($status_spkwofat == 'Approve') {
                $spkwofat_date = date("Y-m-d");
                $status_spkwo = "Approve";

                // Tentukan spkwofat_date
                // Query untuk memperbarui submit_legal dan catatan_owner di tabel sdg_desain
                $sql_update_pending = "UPDATE sdg_desain SET status_spkwofat = ?, catatan_spkwofat = ?, spkwofat_date = ?, status_spkwo = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("ssssi", $status_spkwofat, $catatan_spkwofat, $spkwofat_date, $status_spkwo, $id);
            ...
            } elseif ($status_spkwofat == 'Pending') {
                ...
                // Query untuk memperbarui submit_legal dan catatan_owner di tabel sdg_desain
                $sql_update_pending = "UPDATE sdg_desain SET status_spkwofat = ?, catatan_spkwofat = ?, spkwofat_date = ? WHERE id = ?";
                $stmt_update_pending = $conn->prepare($sql_update_pending);
                $stmt_update_pending->bind_param("sssi", $status_spkwofat, $catatan_spkwofat, $spkwofat_date, $id);
                $stmt_update_pending->execute();
                ...
                echo "Status berhasil diperbarui dan data ditahan.";
                    ...
                    if ($result_sla_stkonstruksi->num_rows > 0) {
                        ...
                        $sla_fat = date("Y-m-d", strtotime($spkwofat_date . ' + ' . $hari_sla_stkonstruksi . ' days'));
                    } else {
                        $conn->rollback();
                        echo "Error: Data SLA tidak ditemukan untuk divisi ST-Konstruksi.";
                        exit;
                    }
                    // Jika status tidak diubah menjadi Approve atau Pending, hanya perlu memperbarui status_spkwofat
                    $sql_update_other = "UPDATE sdg_desain SET status_spkwofat = ?, catatan_spkwofat = ?, spkwofat_date = ? WHERE id = ?";
                    $stmt_update_other = $conn->prepare($sql_update_other);
                    $stmt_update_other->bind_param("sssi", $status_spkwofat, $catatan_spkwofat, $spkwofat_date, $id);
            ...
            header("Location: ../datatables-review-wo-from-sdg.php");
